...
"""
    Credits: dnslib example server fixedresolver. Modified to accomodate TXT records
...
class FixedResolver(BaseResolver):
    ...
    def __init__(self, zone, key_auth_digest="blah"):
        # Parse RRs
        self.rrs = RR.fromZone(zone)
        self.key_auth_digest = key_auth_digest

    def resolve(self, request, handler):
        print('request.q', request.q)
        if request.q.qtype != QTYPE.TXT:
            ...
            print('here')
            reply = request.reply()
            reply.add_answer(RR.fromZone(f'{request.q.qname} 60 IN TXT "{self.key_auth_digest}"')[0])
            return reply
...
if __name__ == '__main__':
    ...
    p.add_argument("--key_auth_digest")
    args = p.parse_args()
    ...
    resolver = FixedResolver(args.response, args.key_auth_digest)
    logger = DNSLogger(args.log, args.log_prefix)

    print("Starting Custom Fixed Resolver (%s:%d) [%s] with key_auth_digest: [%s]" % (
        args.address or "*",
        args.port,
        "UDP/TCP" if args.tcp else "UDP", args.key_auth_digest))
